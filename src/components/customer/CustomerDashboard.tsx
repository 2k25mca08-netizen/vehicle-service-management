    ...
    <div className="bg-blue-100">
      <div className="space-y-12">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {myRecords.filter(r => r.status !== "Ready for Pickup").map((record, idx) => (
              ...
              >
                <Card className={`border-none shadow-sm hover:shadow-xl transition-all duration-300 rounded-[2rem] overflow-hidden group ${record.status === "In Progress" ? "bg-amber-50" : "bg-blue-50"
                  }`}>
                  <CardHeader className="p-8 pb-4">
                    <div className="flex justify-between items-start mb-6">
                      <Badge className={`rounded-xl px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest border-none gap-2 ${record.status === "In Progress" ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700"
                        }`}>
                    ...
                    <CardTitle className={`text-2xl font-display font-bold transition-colors ${record.status === "In Progress" ? "text-amber-950 group-hover:text-amber-700" : "text-blue-950 group-hover:text-blue-700"
                      }`}>
                      {myVehicles.find(v => v.id === record.vehicleId)?.model}
                    </CardTitle>
                    <CardDescription className="font-mono text-xs font-bold text-zinc-500 uppercase">
                      {myVehicles.find(v => v.id === record.vehicleId)?.regNo}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="p-8 pt-4">
                    <div className="p-5 bg-white/60 rounded-2xl border border-white/40">
                      <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mb-2">Current Status</p>
                    ...
                    {record.status === "Booked" && (
                      <Button
                        variant="ghost"
                        size="sm"
                        className="w-full mt-6 text-zinc-500 hover:text-red-700 hover:bg-red-100 rounded-xl font-bold text-xs uppercase tracking-widest h-12 transition-colors"
        ...
        <section>
          ...
          <Card className="border-none shadow-sm rounded-[2.5rem] overflow-hidden bg-indigo-50 border border-indigo-100">
            <CardContent className="p-0">
              <div className="divide-y divide-indigo-100/50">
                {myRecords.filter(r => r.status === "Ready for Pickup").map((record) => (
                  <div key={record.id} className="p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:bg-indigo-100/80 transition-colors group border-b border-indigo-100/50 last:border-0">
                    <div className="flex items-center gap-6">
                      <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm">
                        <CheckCircle2 className="w-7 h-7 text-indigo-600" />
                      </div>
                      <div>
                        <p className="text-xl font-display font-bold text-indigo-950">{myVehicles.find(v => v.id === record.vehicleId)?.model}</p>
                        <p className="text-xs font-bold text-indigo-600/70 uppercase tracking-widest mt-1">
                ...
                {myRecords.filter(r => r.status === "Ready for Pickup").length === 0 && (
                  <div className="p-20 text-center text-indigo-400 italic font-medium">No past records found in your history.</div>
                )}
              </div>
            </CardContent>
          </Card>
